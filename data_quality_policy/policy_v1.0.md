...
A **single cost/execution model** is used across all scenarios.  
**Only commissions differ** by mode:
- **Retail-Standard** — standard retail commission;
- **Retail-Rebate** — retail commission **net** of rebate/cashback;
- **Institutional** — institutional commission.

**The commission schedule is fixed per mode and included in PnL; all other cost elements are identical across modes.**

> Precommitment applies to the data-quality table only. The cost model is unified; only the commission schedule differs by mode. Any future changes to costs will be released as separate results with a new cost-model identifier.
