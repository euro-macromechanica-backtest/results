...
Используется **единая модель издержек** во всех сценариях.  
**Отличается только комиссия** согласно режиму:
- **Retail-Standard** — стандартная розничная комиссия;
- **Retail-Rebate** — розничная комиссия **net** (с учётом ребейта/кэшбэка);
- **Institutional** — институциональная комиссия.

**Комиссия задаётся единожды для каждого режима и включена в расчёт PnL; прочие элементы издержек неизменны для всех режимов.**

> Предкоммитмент распространяется на таблицу качества данных. Модель издержек — единая; отличается только комиссия по режимам. Любые изменения издержек публикуются отдельными результами с новой моделью.
