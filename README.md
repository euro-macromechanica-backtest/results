...
> Заметка для читателей: «квант» — от англ. *quantitative* — «количественно измеряемый».

> Полная инструкция по верификации прозрачности процессов, а также входных и выходных файлов, использовавшихся в бэктесте содержится в [AUDIT.ru.md](https://github.com/euro-macromechanica-backtest/results/tree/main/docs); ссылка продублирована в конце документа.

Результаты проекта демонстрируют **устойчивые эмпирические свидетельства** существования **неизменных квант-факторов (признаков рыночной неэффективности)** в механике движения **EUR/USD** на всём историческом интервале бэктеста. **Стабильно положительные итоги за почти всю историю евро как единой валюты ЕС** (с момента введения наличных в январе 2002 года) — включая ключевые системные кризисы последних двух десятилетий — служат этому подтверждением. Данный проект не «полноценный микро-бэктест» институционального уровня, а **косвенная демонстрация** устойчивой прибыльности полной логики **EMM** через результаты базового ядра M5 при **консервативной модели исполнения**. 

> Уровень проработки методологии, прозрачность пайплайна и применённый статистический инструментарий проекта свидетельствуют о высокой компетентности автора в квант-исследованиях механики цены EUR/USD и макроэкономике.
...
- максимальный учёт всех возможных торговых издержек **при ограниченных** ресурсах, на неинституциональном уровне;
...
Фокус на EUR/USD выбран по **двум причинам**: во-первых, эта пара позволяет реализовать **минимально-достаточное базовое ядро** сигнальной логики полной стратегии EMM для **прозрачной демонстрации** результатов; во-вторых, EUR/USD является **наиболее ликвидной** валютной парой на FX. В отличие от прочих FX-мажоров, структура квант-факторов по EUR/USD позволяет построить **компактную модель без дополнительных эвристик** и при ограниченных ресурсах, при этом сохраняется **статистическая достаточность** профиля результатов в рамках применяемой методики исследования механики цены.
...
- [Публичная логика движка M5 EMM](#m5-emm-engine-logic) 
  ...
  - [Epsilon (ε)](#epsilon)
- [Реализованная логика стратегии M5 EMM](#m5-emm-engine-logic) 
...
- [Аргументация выбора минутных данных HistData для бэктеста M5 EMM](#histdata)
  ...
  - [OTC-специфика и минимальные расхождения M1-данных](#otc) 
...
- [Метрики](#metrics)
  ...
  - [Monte Carlo (EoM)](#monte-carlo)
  - [Доверительные интервалы (BCa)](#confidence-intervals)
  - [Трейд-бутстрэп (EDR & серии убыточных сделок)](#trades-bootstrap)
...
- **Погрешность из-за 5-минутного разрыва**: поскольку **Objective (TP)** пересчитывается на каждом новом M5, его значение после разрыва может немного отличаться от гипотетического уровня без разрыва — как в «плюс», так и в «минус». Соответсвенно, это не даёт несанкционированного преимущества стратегии.
...
> **Подробности о методологии модели издержек читать в соотвествующем разделе `Методология модели издержек (EUR/USD) M5 EMM`**.
...
**Spread и slippage** заданы явно и учитываются на этапе лот-сайзинга: дистанция от входа до SL берётся уже с учётом этих издержек, поэтому сам «стоп» не повышает риск сверх целевого.
...
### Реализованная логика M5 EMM охватывает базовое ядро — около 10–15% полной стратегии EMM. Торговля, математические расчёты и учёт факторов выполняются исключительно на 5-минутных таймфреймах. Реализация логики для M5 неполна: её сложность не позволяет внедрить полный набор квант-факторов даже в рамках одного 5-минутного таймфрейма при ограниченных ресурсах разработки и данных. Кроме того, текущая реализация M5 не охватывает торговлю основной логики в пост-окнах после выхода важных календарных релизов, что является немаловажным драйвером результативности EMM. Тем не менее это не мешает продемонстрировать результативность базового ядра M5.
...
Реализованная стратегия опирается на торговлю макроструктурой на M5-барах и торгует только **в спокойные интервалы** торговых окон. Реализованные временные фильтры на запрет торговли фиксированы в UTC. Логика торгует макроструктурой на **M5** только в спокойные интервалы. **Это не «только Азия»**: под «спокойными» понимаются окна вне волатильных интервалах в начале каждого часа сессий **London–NY**, ролловера и релизов; фильтры применяются и в активных отрезках **London–NY**. Все запреты заданы **фиксированными окнами в UTC** и **не зависят от DST** (источник данных HistData с фиксированной EST-привязкой учтён; ссылка на F.A.Q. с форматом времени — `http://www.histdata.com/f-a-q/`).
...
> **Комиссия (институциональные условия, EUR/USD).** По умолчанию применяется **$5.5 за round-turn на 1 стандартный лот (100 000 EUR)** как отдельная строка расходов, эквивалентная **≈ 0.55 пипса RT** (≈ $2.75 per side; **$55/$1M RT**). Spread и slipppage учитываются раздельно в соответствии с моделью исполнения. Диапазон рыночных условий для ECN/PoP составляет **$4–6 RT/лот**; для all-in-потоков стоимость содержится в спреде, однако в целях сопоставимости применяется единая явная комиссия.
...
Данные HistData может скачать любой напрямую с официального сайта и сверить их целостность с данными, использованными в бэктесте. Минутные и тик-ряды HistData сопровождаются отчётами по разрывам, что обеспечивает **прозрачность и воспроизводимость** входных данных для любого, кто верифицирует пайплайн.

При использовании данных **Dukascopy** для охвата полного горизонта бэктеста (десятилетия) требуются сторонние скрипты и библиотеки для скачивания данных, так как интерфейс источника не позволяет скачивать данные с охватом больших периодов, что, в свою очередь, ухудшает **прозрачность и воспроизводимость** исходных данных для **бэктеста**.
...
В качестве референса качества публичных тик-данных использованы тик-ряды HistData за случайно выбранные месяцы и годы с отчётами по разрывам — [data-hub/source_data/tick_data_reference](https://github.com/euro-macromechanica-backtest/data-hub/tree/main/source_data/tick_data_reference). Любое третье лицо может непосредственно загрузить тик-данные за произвольные периоды с официального сайта провайдера, воспроизвести процедуру анализа (детекция и статистика разрывов) и, тем самым, независимо верифицировать характеристики качества данных и соответствующие выводы.

**Качество тиковых данных HistData (пример: 2015-09)**  
По отчёту за месяц выявлено **178 разрывов** тикового потока суммарной длительностью **13 262 с** (≈ **3:41:02**). Длительность «дыр» варьирует **60–186 с** (медиана **67 с**, среднее **74.51 с**), преобладают пропуски **60–90 с**. Средний интервал между тиками, заявленный в отчёте, составляет **5 247 мс**, максимальный — ≈ **186 250 мс**.  
Указанные разрывы означают, что тиковый ряд **не является lossless** и не позволяет надёжно восстанавливать порядок событий внутри минут/пятиминуток, где действует интрабар-логика. С учётом методики (**M5 EMM**, детерминированные правила разрешения конфликтов High/Low, учёт издержек) использование тиков HistData **не даёт статистически значимого преимущества** над минутными рядами HistData, при этом **ухудшает прозрачность и воспроизводимость**.  
В работе применяются минутные **OHLC HistData** и фиксированные правила исполнения; тиковые данные HistData по сравнению с минутными данными того же провайдера **не улучшают качество верификации** результатов и **не рекомендованы** для данного проекта. Это демонстрирует **отсутствие практической пользы** при проведении тикового бэктеста логики **M5 EMM** на этих данных.

**Обоснование выбора минутных данных, а не тик-данных**

При реализованной логике **M5 EMM** применение тик-данных публичного доступа с высокой вероятностью **не приведёт к статистически значимому** изменению результатов бэктеста по сравнению с минутными рядами, при том что усложнит пайплайн и снизит воспроизводимость — **даже если** их качество у Dukascopy локально выше, чем у референса HistData. 
...
- **Пропуски в L1-потоках.** В публичных **L1** тик-данных (HistData, Dukascopy и др.) систематически встречаются длительные межтиковые пропуски — нередко минутного масштаба; такой поток **не lossless** и без строгой нормализации/аудита (UTC, дедупликация, gap-report) **не пригоден** для объективного восстановления порядка событий внутри баров.
- **Технологическая нагрузка и риски верифицируемости.** Тики Dukascopy требуют нормализации и очистки (дубликаты, «сердцебиения», пропуски, стыки часовых файлов). Это повышает шум и создаёт риск невоспроизводимой обработки, что затрудняет независимую проверку.
...
<a id="otc"></a>
### OTC-специфика и минимальные расхождения M1-данных

**Факт.** Рынок spot-FX является **OTC**; единого центра формирования цены не существует. Поэтому **межпровайдерные микрорасхождения** на минутных данных (M1) неизбежны и обусловлены различиями в составе LP, тайм-зонах и разметке минут (включая DST), типе цены (Bid/Ask/Mid/Last), правилах агрегации и фильтрации. **В частности, минутные ряды HistData и Dukascopy могут микроскопически отличаться по OHLC отдельных минут** по указанным причинам; после корректной нормализации такие расхождения имеют **операционно несущественный** характер для реализованной стратегии M5 EMM.

**Случаи, когда это существенно.** Для конфигураций с **ультра-короткими стопами 2–5 пипсов** (скальпинг, триггеры на границе бара) указанные расхождения способны **материально влиять** на факты входа/выхода и итоговый P&L. 
...
Для стратегии со **значительно более широкими стоп-уровнями** (как в реализованной логике M5 EMM), **невысокой частотой сделок** (что подтверждается метриками) и **исключением «тонких» интервалов** (новости, ролловеры, волатильные интервалы в начале каждого часа London/NY) M1-расхождения **операционно пренебрежимы** при соблюдении следующего:

- **Нормализация данных.** Все ряды приведены к **UTC** и используют **единую ценовую базу**; по всему горизонту применяется **единая методика**.
- **Обращение с пропусками.** **Разрывы не заполняются** интерполяцией; реальные паузы учитываются логикой обработки разрывов.
- **Фильтры ликвидности.** Применяются **идентичные правила** исключения новостных окон, ролловеров и волатильных интервалов в начале каждого часа (London/NY), т. е. периодов максимальных M1-расхождений.
- **Параметры уровней.** **Guard (SL)/Objective (TP) не микроскопические** — порядка **десятков пипсов** и **не меньше** типичной внутриминутной волатильности; поэтому сдвиги на **±0.1–0.2 пипса** по **High/Low** редко меняют факт входа/выхода (это учтено реализацией **epsilon**, описанной в разделе `Публичная логика движка M5 EMM`).
- **Единые правила исполнения.** На всём горизонте действует **единое правило** для **Objective (TP)/Guard (SL)**; при одновременном конфликте в одной минуте приоритет у **Guard (SL)**.
- **Модель издержек.** Издержки заданы **экзогенно**: **spread + slippage** как **round-turn**, **комиссия** — отдельной строкой; параметры согласованы с объёмами сделок по **EUR/USD**.
- **Горизонт и частота.** **Длинный исторический период (десятилетия)** и **низкая частота сделок** усредняют локальные микро-дельты M1, минимизируя влияние на агрегированные метрики.
- **Анализ чувствительности.** Робастность, продемонстрированная в результатах, подтверждает устойчивость метрик к малым вариациям входных данных при возмущениях **±0.10 пипса (round-turn)**.

**Итог.** Небольшие различия между источниками M1 — естественная особенность OTC-FX. HistData и Dukascopy могут давать **микроскопически отличающиеся** минутные бары, однако при корректной нормализации данных это **не влияет** на результаты стратегии M5 EMM.

---

<a id="m1-conclusion"></a>
### Вывод по выбору данных
Ухудшать прозрачность и воспроизводимость за счёт использования скриптов и библиотек для скачивания минутных рядов, тик-данных из Dukascopy и усложнения пайплайна анализа пригодности и нормализации **не является эффективным решением** для поставленных задач. Рациональный выбор — минутные данные **HistData** с детерминированными правилами интрабар-исполнения и явно заданной моделью издержек.

> **Примечание.** «По-настоящему подробный» микробэктест — с честной очередностью, частичными исполнениями, реалистичной ликвидностью и спред-микродинамикой, точным порядком событий — **возможен только на институциональных данных** (EBS / Cboe FX / Refinitiv и др.), где доступны **L2/L3**, события книги заявок (**add/modify/cancel/trade**), **микросекундные/миллисекундные** метки и **lossless**-съём.
...
Соответсвенно, результаты воспроизводятся бит-в-бит при одинаковых входных данных с соблюдением версий Python, библиотек и зависимостей которые использует движок бэктеста.
...
- верифицируется **SHA-256-хэш** таблицы модели издержек [`m5_emm_cost_model_v1.0.csv`](https://github.com/euro-macromechanica-backtest/results/blob/main/docs/cost_model/m5_emm_cost_model_v1.0.csv) и сверяются выбранные **для прогона** значения издержек для соответствующего режима и календарного года;
...
Live-прогоны зафиксированы как **непрерывные, несмонтированные записи экрана** (one-take; без склеек, кадрирования, ускорений и иной пост-обработки) публикуются исходные файлы с **SHA-256** для независимой проверки целостности.
...
- анализ на **длинной исторической выборке**;
- **роллинг-оценки** на окнах **12/36 месяцев**;
- оценка **квантилей просадок**;
- результаты **трейд-бутстрэпа (EDR и серии убыточных сделок)**
- **BCa-доверительные интервалы**;
- **Monte Carlo** с **стационарным бутстрэпом (stationary bootstrap)** **по всем заявленным конфигурациям**.
...
Робастность продемонстрирована на профилях и в режимах капитализации на основе динамической модели издержек.

**Retail Standard:** Fixed Start 100k и Compounding EoY–SoY Base 100k.  
**Retail Rebate:** Fixed Start 100k и Compounding EoY–SoY Base 100k.

В качестве детального примера робастности проиллюстрированы **Institutional-профили**, где издержки фиксируются по диапазонам **notional volume**:
...
- Результаты относятся к одной паре (**EUR/USD**) и одной частоте (**M5**).
- Исполнение моделируется на **M1** с консервативной моделью издержек; **тик-уровень** не моделируются.
- Качество исторических данных **неоднородно** (см. Data Quality Policy v1.0); стресс-периоды не входят в сводные метрики.
...
- [Полная инструкция по верификации прозрачности процессов, а также входных и выходных файлов, использовавшихся в бэктесте](https://github.com/euro-macromechanica-backtest/results/tree/main/docs)
...
- [Коммерческое сотрудничество](https://github.com/euro-macromechanica-backtest/results/blob/main/COMMERCIAL.md)
...
- Карта лицензий – [LICENSES.md](https://github.com/euro-macromechanica-backtest/results/blob/main/LICENSES.md)
