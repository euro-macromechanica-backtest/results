...
**OOS/Walk‑Forward are not applied**: a **single fixed M5 EMM logic** is used **without re‑optimization/curve‑fitting**; the purpose of OOS is to detect overfitting after tuning, **which is not present here**. **Multiple testing bias is absent**: a **single hypothesis/strategy** was tested without parameter sweeps (selection bias is minimal). **Profile robustness is confirmed** by the **long history**, **12/36m rollings**, **drawdown quantiles**, **BCa intervals**, and **Monte Carlo** (stationary bootstrap **across all stated configurations**). **For audit upon request**, a **holdout** without retraining and without parameter changes can be prepared for independent verification.
